...
public abstract class CITCache<T extends CITType> {
    ...
    public static class Single<T extends CITType> extends CITCache<T> {
        ...
        public WeakReference<CIT<T>> get(CITContext context) {
            if (this.cit == null)
                this.lastCachedStamp = System.currentTimeMillis() + CITRebornConfig.INSTANCE.cache_ms + (offset += 4);

            if (this.cit == null || System.currentTimeMillis() - this.lastCachedStamp >= CITRebornConfig.INSTANCE.cache_ms) {
