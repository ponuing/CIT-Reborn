 ...
 * @see CITRebornConfig
 */
public class CITRebornConfigScreenFactory {
    /**
    ...
    public static Screen create(Screen parent) {
        CITRebornConfig currentConfig = CITRebornConfig.INSTANCE, defaultConfig = new CITRebornConfig();

        ConfigBuilder builder = ConfigBuilder.create()
                .setParentScreen(parent)
                .setTitle(Text.translatable("config.citreborn.title"))
                .setSavingRunnable(currentConfig::write);

        ConfigCategory category = builder.getOrCreateCategory(Text.empty());
        ConfigEntryBuilder entryBuilder = builder.entryBuilder();

        category.addEntry(entryBuilder.startBooleanToggle(Text.translatable("config.citreborn.enabled.title"), currentConfig.enabled)
                .setTooltip(Text.translatable("config.citreborn.enabled.tooltip"))
        ...
        if (FabricLoader.getInstance().isModLoaded("citreborn-defaults")) {
            ...
            category.addEntry(entryBuilder.startBooleanToggle(Text.translatable("config.citresewn-defaults.title"), false)
                    ...
                    .setYesNoTextSupplier((b) -> {
                        ...
                        return Text.translatable("config.citreborn.configure");
                    })
                    .build());
        }

        category.addEntry(entryBuilder.startBooleanToggle(Text.translatable("config.citreborn.mute_errors.title"), currentConfig.mute_errors)
                .setTooltip(Text.translatable("config.citreborn.mute_errors.tooltip"))
                .setSaveConsumer(newConfig -> currentConfig.mute_errors = newConfig)
                .setDefaultValue(defaultConfig.mute_errors)
                .build());

        category.addEntry(entryBuilder.startBooleanToggle(Text.translatable("config.citreborn.mute_warns.title"), currentConfig.mute_warns)
                .setTooltip(Text.translatable("config.citreborn.mute_warns.tooltip"))
                .setSaveConsumer(newConfig -> currentConfig.mute_warns = newConfig)
                .setDefaultValue(defaultConfig.mute_warns)
                .build());

        category.addEntry(entryBuilder.startIntSlider(Text.translatable("config.citreborn.cache_ms.title"), currentConfig.cache_ms / 50, 0, 5 * 20)
                .setTooltip(Text.translatable("config.citreborn.cache_ms.tooltip"))
                ...
                .setTextGetter(ticks -> {
                    ...
                    return Text.translatable("config.citreborn.cache_ms.ticks.any", ticks).formatted(color);
                })
                .build());

        category.addEntry(entryBuilder.startBooleanToggle(Text.translatable("config.citreborn.broken_paths.title"), currentConfig.broken_paths)
                .setTooltip(Text.translatable("config.citreborn.broken_paths.tooltip"))
