...
public class GlobalProperties extends PropertyGroup {
    ...
    public void callHandlers() {
        for (EntrypointContainer<CITGlobalProperties> container : FabricLoader.getInstance().getEntrypointContainers(CITGlobalProperties.ENTRYPOINT, CITGlobalProperties.class)) {
            ...
            if (containerNamespace.equals("pcit") || containerNamespace.equals("citresewn-defaults") || containerNamespace.equals("citreborn") || containerNamespace.equals("citreborn-defaults"))
            ...
            for (Map.Entry<PropertyKey, Set<PropertyValue>> entry : properties.entrySet())
                if (entry.getKey().namespace().equals(containerNamespace)) {
                    ...
                    try {
                        ...
                        CITReborn.logErrorLoading(lastValue == null ? "Errored while disposing global properties" : "Errored while parsing global properties: Line " + lastValue.position() + " of " + lastValue.propertiesIdentifier() + " in " + lastValue.packName());
                        e.printStackTrace();
                    }
                }
        }
