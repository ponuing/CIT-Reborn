...
/**
 * Logic for the /citreborn client command. Only enabled when Fabric API is present.<br>
 * Structure:
 * <pre>
 * /citreborn - General info command
 * /citreborn config - Opens the config gui(only when Cloth Config is present)
 * /citreborn analyze pack &lt;pack&gt; - Displays data for the given loaded cit pack.
 * </pre>
 */
public class CITRebornCommand {
    /**
     * @see com.ponuing.mixin.ChatScreenMixin
     */
    public static boolean openConfig = false;

    /**
     * Registers all of CIT Reborn's commands.
     */
    public static void register() {
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(
                ClientCommandManager.literal("citreborn").executes(context -> {
                    context.getSource().sendFeedback(of("CIT Reborn v" + FabricLoader.getInstance().getModContainer("citreborn").orElseThrow().getMetadata().getVersion() + ":"));
                    ...
                    final boolean active = CITRebornConfig.INSTANCE.enabled && ActiveCITs.isActive();
                    context.getSource().sendFeedback(of("  Active: " + (active ? "yes" : ("no, " + (CITRebornConfig.INSTANCE.enabled ? "no cit packs loaded" : "disabled in config")))));
                    if (active) {
                ...
                .then(literal("config")
                        .executes(context -> { //CITReborn config
                            openConfig = true;
                ...
                .then(literal("analyze")
                        .then(literal("pack")
                                .then(argument("pack", new LoadedCITPackArgument())
                                        .executes(context -> { //CITReborn analyze <pack>
    ...
    /**
     * Greedy string argument that is limited to cit pack names loaded in {@link com.ponuing.cit.ActiveCITs}.
     */
