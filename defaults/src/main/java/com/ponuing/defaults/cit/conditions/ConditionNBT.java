...
public class ConditionNBT extends CITCondition {
    ...
    public boolean test(CITContext context) {
        ...
        if (testPath(root, 0, context)) {
            return true;
        }
        NbtCompound customData = LegacyNbtUtils.extractCustomData(root);
        if (customData != null && testPath(customData, 0, context)) {
            return true;
        }
        NbtCompound tag = LegacyNbtUtils.extractTag(root);
        return tag != null && testPath(tag, 0, context);
    }
    ...
    public boolean testString(String element, Text elementText, CITContext context) {
        if (element != null) {
            ...
            if (elementText == null)
                elementText = Text.Serialization.fromJson(element, context.world.getRegistryManager());
        }
